...
- **`resolveAddr` refuses address 0** (`port.go:39-42`) — legitimate GVCP/ABRM addresses; also truncates 64→32-bit (`port.go:43`), dropping `0xFFFF…` ConfRom/SmartFeature space (§2.8.14/15).
- **Floating nodes never stored** — `<Integer><Value>` fails on Set/Read because node has no address (`nodemap.go:161-167`); same for floating String/Float/Converter (`nodemap.go:286-318`).
- **Runtime access mode never derived/enforced.** §2.5 defines NI/NA/WO/RO/RW. Impl: `GetAccess()` returns raw XML string (`node.go:113-115`); writes don't check access/locked/available (`nodemap.go:80-86,133-167`); RO write or WO read unguarded; `ImposedAccessMode` (§2.8.1) unparsed.
...
- **Node elements**: `pLength`, `pPort`, `pValueCopy` (§2.8.5), `ValueIndexed/pValueIndexed/ValueDefault/pValueDefault` (§2.8.5/9), `ValidValueSet`, `Unit`, `Representation`, `DisplayNotation`, `DisplayPrecision`, `PollingTime`, `ImposedAccessMode`, `Visibility`, `NameSpace`, `MergePriority`/merge (§2.8.1), `pError`, `pAlias/pCastAlias`, `IsDeprecated`, `EventID` invalidation, `pBlockPolling`.
- **Enumeration** (§2.8.10): `NumericValue`, `IsSelfClearing`, `Symbolic`, EnumEntry `pIsImplemented/pIsAvailable`, polling semantics.
- **SwissKnife** (§2.8.13): `Constant`, `Expression` sub-exprs, variable suffixes `.Min/.Max/.Inc/.Value/.Entry.Name` missing; all math evaluated as `int64` (`port.go:237-240`), e.g. `(Gain.Max-Gain.Min)/2` truncates; required ops `==`/`**` + float funcs now added (§7 #9).
