...
- **SwissKnife** (§2.8.13): `Constant`, `Expression` sub-exprs, `.Entry.Name` qualified suffix; all math evaluated as `int64` (`port.go:237-240`), e.g. `(Gain.Max-Gain.Min)/2` truncates — variable suffixes `.Min/.Max/.Inc/.Value/.Entry` resolved (`evaluator.go` + `formulaContext`), but float-domain formulas (e.g. `GainRawMaxExpr` = `VAR_GAINMAX`, a Float feature) still error because `pVariable` targets are evaluated in the integer register domain only; required ops `==`/`**` + float funcs now added (§7 #9).
